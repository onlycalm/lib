...
#if LOG_LV >= FATAL
#define PrtLogFat(Str, ...)         printf(FmtLog(FATAL) Str "\n", ##__VA_ARGS__)
#else
#define PrtLogFat()                 ((void)0u)
#endif //LOG_LV

#if LOG_LV >= ERROR
#define PrtLogErr(Str, ...)         printf(FmtLog(ERROR) Str "\n", ##__VA_ARGS__)
#else
#define PrtLogErr()                 ((void)0u)
#endif //LOG_LV

#if LOG_LV >= WARN
#define PrtLogWrn(Str, ...)         printf(FmtLog(WARN) Str "\n", ##__VA_ARGS__)
#else
#define PrtLogWrn()                 ((void)0u)
#endif //LOG_LV

#if LOG_LV >= DEBUG
#define PrtLogDbg(Str, ...)         printf(FmtLog(DEBUG) Str "\n", ##__VA_ARGS__)
#else
#define PrtLogDbg()                 ((void)0u)
#endif //LOG_LV

#if LOG_LV >= TRACE
#define PrtLogTr(Str, ...)          printf(FmtLog(TRACE) Str "\n", ##__VA_ARGS__)
#define PrtLogEntFun()              printf(FmtLog(TRACE) "<-----Enter function----->\n")
#define PrtLogExFun()               printf(FmtLog(TRACE) "<-----Exit function----->\n")
#else
#define PrtLogTr()                  ((void)0u)
